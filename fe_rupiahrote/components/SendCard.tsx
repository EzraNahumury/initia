...
export function SendCard() {
  ...
  // Register username — precheck so users don't waste gas on a taken name.
  const registerFullName = registerName
    ? (registerName.endsWith(".init") ? registerName : `${registerName}.init`)
    : "";
  const { data: registerResolved } = useReadContract({
    address: FAUCET_ADDRESS,
    abi: FAUCET_ABI,
    functionName: "resolveUsername",
    args: [registerFullName],
    query: { enabled: registerFullName.length > 5 },
  });
  const registerTaken = !!registerResolved && registerResolved !== "0x0000000000000000000000000000000000000000";
  ...
  const handleRegister = () => {
    if (!registerName || registerTaken) return;
    writeRegister({
      address: FAUCET_ADDRESS,
      abi: FAUCET_ABI,
    ...
    <div className="flex gap-4 items-start">
      ...
      <div className="glass rounded-2xl glow-purple-sm p-5 space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleRegister} disabled={!registerName || isRegistering || isRegConfirming || registerTaken}
            className={`px-4 py-2.5 rounded-xl text-[8px] font-semibold transition-all cursor-pointer ${
              !registerName || isRegistering || isRegConfirming || registerTaken
                ? "bg-purple/20 text-text-muted cursor-not-allowed"
                : "bg-purple text-white hover:bg-purple-light"
            }`}>
            {isRegistering || isRegConfirming ? "Registering..." : registerTaken ? "Taken" : "Register"}
          </button>
        </div>
        {registerTaken && (
          <div className="flex items-center gap-1.5 text-[7px] text-red font-medium">
            <HiXCircle className="w-3 h-3" />{registerFullName} is already registered
          </div>
        )}
        {isRegSuccess && (
          <div className="text-[8px] text-green font-medium">Username registered! Others can now send to {registerFullName}</div>
        )}
